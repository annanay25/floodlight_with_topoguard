...
public class LinkDiscoveryManager implements IOFMessageListener,
    ...
    protected Map<Long, Map<Short, byte[]>>  macMap;
  ...
  */    
    ...
    protected Command handlePacketIn(long sw, OFPacketIn pi,
        ...
        /*
        	...
        	Command command = handleLldp((LLDP) eth.getPayload(), sw, pi.getInPort(), true, cntx);
        	time8 = System.nanoTime();
        	
        	 String str = time5.toString() + ','
						+ time6.toString() + ','
						+ time7.toString() + ','
						+ time8.toString();

		//evaluation: write the time to file
		File f = new File("/mnt/hgfs/share_folder/evaluation/time_overhead_lldp_verification.csv");
		Writer fileWriter = null;
		BufferedWriter bufferedWriter = null;
		if(f.exists() && !f.isDirectory() && time6 != 0 && time7 !=0){
		try {
		fileWriter = new FileWriter(f,true);
		bufferedWriter = new BufferedWriter(fileWriter);
	...
	log.info("[handleLldp] LLDP packet received: " + lldp.getOptionalTLVList().toString() + " at " + time5.toString());
        ...
        if (remoteDpid == null || remotePort == null){
        	return Command.STOP;
        }
   ...
   //TopoSec exntension
    /*
     * Calculate Message Authentication Code value
     */
    protected byte[] generateMACTLVValue(Long dpid, Short port){
    	//if MAC value for <dpid,portid> is pre-storage, just use it
    	if (macMap.containsKey(dpid)){
    		if (macMap.get(dpid).containsKey(port)){
    			return macMap.get(dpid).get(port);
    		}
    	}
    	try{
    	Map<Short, byte[]> macValue = new HashMap<Short, byte[]>();
    		
    	String msg = dpid.toString() + port.toString();
		
		byte[] msgB = msg.getBytes("UTF-8");
		
		byte[] macTLVValue = this.mac.doFinal(msgB);
		if (!macMap.containsKey(dpid)){
			macValue.put(port, macTLVValue);
			macMap.put(dpid, macValue);
		}
		else{
			if(!macMap.get(dpid).containsKey(port)){
				macValue = macMap.get(dpid);
				macValue.put(port, macTLVValue);
				macMap.put(dpid, macValue);
			}
		}
		...
		log.error("Generate MAC TLV error: Invalid Key: {}", e.getMessage());
		return null;
   ...
   /*
    * Add Message Authentication Code TLV into LLDP packet
    */
    ...
    public void init(FloodlightModuleContext context)
        ...
        try{
        	...
        	macMap = new HashMap<Long, Map<Short, byte[]>>();
        	
        }
     	catch (NoSuchAlgorithmException e){
    		log.error("Generate MAC TLV error:No {} Algorithm", e.getMessage());
    	} 
    	catch (InvalidKeyException e) {
    		log.error("Generate MAC TLV error: Unsupported Encoding {}", e.getMessage());
    	} 
